...
public:
    ...
    Ray generateRay(const Vector2f &point) override {
        Vector3f drc(point.x() - width/2, height/2 - point.y(), fxy * (distance + 1)); drc.normalize();
        Vector3f drw = drc.x() * horizontal - drc.y() * up + drc.z() * direction;      drw.normalize();
